			...
			for (var j = 0; j <= tiempo; j++) {
				...
				if (j == 0) {
					...
					if (
						...
						if (this.auxbecares != undefined && j > 0 && j <= this.auxbecares) {
							...
							this.total_pagar =
								this.total_pagar + this.pensionesestudiante[this.idexpension].val_beca;
						} else {
							if (j != 0) {
								//  //console.log('Pension',this.config.pension);
								this.total_pagar = this.total_pagar + this.config.pension;
				...
				if (j == 0) {
					this.pagopension.push({
						...
						pago: idpago,
						tipo: tipo,
						porpagar:this.pensionesestudiante[this.idexpension].idanio_lectivo.matricula-valor
					});
				} else {
					...
					if(this.pensionesestudiante[this.idexpension].condicion_beca=='Si'){
						this.arr_becas.forEach((element:any) => {
							if(new Date(new Date(f).setMonth(new Date(f).getMonth() + j - 1)).getTime()==new Date(element.titulo).getTime()){
