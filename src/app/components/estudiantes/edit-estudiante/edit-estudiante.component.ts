...
export class EditEstudianteComponent implements OnInit {
					...
					if (response.data == undefined) {
						...
						this._adminService.obtener_config_admin().subscribe((response) => {
							...
							if(response.data){
								...
								this.valores_pensiones = response.data.pension;
								...
								this._estudianteService.obtener_pension_estudiante_guest(this.id, this.token).subscribe((response) => {
									...
									if(response.data){
										...
										Object.assign(this.estudiante,pen[pen.length-1]);
										...
										console.log(this.pension);
											for(var i=0;i<10; i++){
												...
												if(i>this.estudiante.meses-1 && new Date(this.estudiante.idanio_lectivo.mescompleto).getMonth()!=new Date (this.pension.anio_lectivo).getMonth()+i){
													this.etiquetas.push({
														etiqueta:i+1,
														titulo: new Date (new Date (this.pension.anio_lectivo).setMonth(new Date (this.pension.anio_lectivo).getMonth()+i) ),
													...
													.subscribe((response) => {
														...
														if(response.abonos){
															...
															this.pagos.forEach((element) => {
																...
																if(element.idpension == this.pension._id && element.abono==0 && element.tipo!=0 ){
	...
	descuento_valor(id) {
		...
		if (
			...
			this.pension.val_beca='0';
			this.pension.desc_beca='0';
		...
		if (updateForm.valid) {
			...
			if (this.estudiante.condicion_beca == "Si") {
				...
				if (this.estudiante.desc_beca == undefined) {
					this.estudiante.desc_beca = this.pension.desc_beca;
				}
				if (this.estudiante.val_beca == undefined) {
					this.estudiante.val_beca = this.pension.val_beca;
				}
