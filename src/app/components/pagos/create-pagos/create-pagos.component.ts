...
export class CreatePagosComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this._adminService.obtener_config_admin().subscribe((response) => {
			...
			this.num_pagos = response.data.numpension;
			this.valores_pensiones = parseFloat(response.data.pension.toFixed(2));
			this.aux_valor_pension = parseFloat(response.data.pension.toFixed(2));
			this.valor_matricula = parseFloat(response.data.matricula.toFixed(2));
			this.mes = response.data.anio_lectivo;
			//////console.log(this.mes);

			this.auxmes = response.data.anio_lectivo;
		});
	}
	...
	select_estudiante(item: any) {
		...
		this._estudianteService
			...
			.subscribe((response) => {
				...
				for (var i = this.pension.length-1; i >= 0; i--) {
					
					//console.log(this.pension[i]);
					
					if (this.pension[i].meses < 9 || (this.pension[i].matricula != 1&&this.pension[i].paga_mat==0)) {
	...
	actualizar_valor() {
		...
		this._adminService
			...
			.subscribe((response) => {
				...
				var auxmeses;
				if (this.selec_est.f_desac != undefined && this.selec_est.estado == "Desactivado" && 
	...
	addDocumento2(debe: any) {
		...
		if (this.valor >= 0.001) {
			...
			//////console.log(this.valor);
			//////console.log(this.valorigualdocumento);
			//////console.log(parseFloat(this.documento_select.valor));
			if (parseFloat((parseFloat(this.documento_select.valor) - this.valorigualdocumento).toFixed(2)) > 0) {
				this._adminService
					...
					.subscribe((response) => {
						...
						if (
							parseFloat((parseFloat(this.documento_select.valor) - this.valorigualdocumento).toFixed(2)) > 0 &&
							debe - (auxb + auxc) > 0
						) {
	...
	registrar_pago() {
		...
		this._adminService.obtener_admin(aux, this.token).subscribe((response) => {
			...
			if (
				this.rol == undefined ||
				this.rol.estado == "Fuera" ||
				this.rol.estado == "deshabilitado"
			) {
				iziToast.show({
					title: "ERROR",
					titleColor: "#FF0000",
