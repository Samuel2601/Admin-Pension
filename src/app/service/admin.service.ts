...
export class AdminService {
	...
	facturacion(wsdl: any): Observable<any> {
		...
		return this._http.post("http://181.113.65.229:7071/WS/Facturador?"+ wsdl, { headers: headers });
	}
	...
	listar_pensiones_estudiantes_tienda(token: any): Observable<any> {
		let headers = new HttpHeaders({
			"Content-Type": "application/json",
			Authorization: token
		});
		return this._http.get(this.url + "listar_pensiones_estudiantes_tienda", {
			headers: headers
		});
	}
	...
	obtener_detallespagos_admin(token: any): Observable<any> {
		let headers = new HttpHeaders({
			"Content-Type": "application/json",
			Authorization: token
		});
		return this._http.get(this.url + "obtener_detallespagos_admin", { headers: headers });
	}
